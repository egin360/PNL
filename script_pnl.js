...
function loadAdminPanel() {
    ...
    alarmsRef.on('value', (alarmsSnapshot) => {
        ...
        for (const deviceId in alarmsData) {
            ...
            // --- Lógica para el círculo de estado y la señal WiFi ---
            let circleClass = 'offline';
            if (deviceData.is_online) circleClass = 'online';
            if (deviceData.ringring) circleClass = 'ringing';
            
            const rssiText = deviceData.is_online ? `${deviceData.wifi_rssi || 'N/A'} dBm` : '--- dBm';
            
            let signalClass = 'wifi-signal-disconnected'; // Clase por defecto si está offline
            if (deviceData.is_online && deviceData.wifi_rssi !== undefined) {
                signalClass = getSignalStrengthClass(deviceData.wifi_rssi);
            }

            // --- Estructura HTML con las barras de WiFi ---
            deviceCard.innerHTML = `
                <span class="device-name">${deviceId}</span>
                <div class="status-indicator">
                    <div class="status-circle ${circleClass}"></div>
                    <span class="rssi-text">${rssiText}</span>
...
// --- FUNCIÓN AÑADIDA PARA CALCULAR LA CLASE DE SEÑAL WIFI ---
